...
const typed = new Typed('#dynamic-text', {
	strings: [
	  ...
	  'Identificar y predecir tendencias',
	  'Buyer Persona',
	  'Salud de marca',
	],
	typeSpeed: 50,
...
$(document).ready(function() {
	$('a[data-toggle="tab"]').on('shown.bs.tab', function(event) {
	  var target = $(event.target).attr('href'); // Obtiene el atributo href del enlace de la pestaña activa
	  $('html, body').animate({
		scrollTop: $(target).offset().top
	  }, 1000); // Ajusta la duración de la animación (1000 ms = 1 segundo)
	});
  });
/*cursor*/
...
document.addEventListener('DOMContentLoaded', function () {
	const rellax = new Rellax('.parallax-video', {
	  speed: -2,
	  center: true,
	  wrapper: null,
	  round: true,
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    video.playbackRate = 0.8; // Ajusta este valor según la velocidad de reproducción deseada
});
